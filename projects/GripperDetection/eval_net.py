...
def eval_sequence(cfg, model, sequence: str, visualize_output=True, build_trajs=True, only_build_first_traj=False):
    test_data_loader = DefaultTrainer.build_test_loader(cfg, sequence)
    test_data_loader = GripperDetectionDatasetWrapper(test_data_loader.dataset)

    with torch.no_grad():
        outputs = model(test_data_loader)

    seq_name = str.join('_', str(test_data_loader[0]['image_id']).split('_')[:-4])
...
def main(args, visualize_output=True, build_trajs=True, only_build_first_traj=False, sequence=""):
    cfg = setup_cfg(args)
    ...
    if sequence == "":
        ...
        for i in range(NUM_SEQUNECES):
            eval_sequence(cfg, model, f"irl_kitchen_gripper_detection_cam_1_seq_{i:03d}", visualize_output=visualize_output,
                          build_trajs=build_trajs, only_build_first_traj=only_build_first_traj)
            eval_sequence(cfg, model, f"irl_kitchen_gripper_detection_cam_2_seq_{i:03d}", visualize_output=visualize_output,
                          build_trajs=build_trajs, only_build_first_traj=only_build_first_traj)
    else:
        eval_sequence(cfg, model, sequence=sequence, visualize_output=visualize_output,
                      build_trajs=build_trajs, only_build_first_traj=only_build_first_traj)


if __name__ == "__main__":
    args = default_argument_parser().parse_args()
    main(args, visualize_output=True, build_trajs=True, only_build_first_traj=False,
         sequence="irl_kitchen_gripper_detection_cam_1_seq_099")
